...
- (BOOL)checkGestureResult {
    ...
    // 返回结果
    return [result isEqualToString:self.password];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if(self.finished)
    {
        if(_blsetPassWord)  //设置手势
        {
            if (self.delegate_setLock && [self.delegate_setLock respondsToSelector:@selector(lockView:didFinishPath:)])
            {
                ...
                [self.delegate_setLock lockView:self didFinishPath:path];
            }
        }
        else                //解锁手势
        {
            if([self checkGestureResult])
            { // 解锁成功
            
                [[UIColor blueColor] set];
                
                // 如果实现了解锁成功的代理方法
                if([self.delegate_Lock respondsToSelector:@selector(gestureViewUnlockSuccess:)]) {
                    
                    [self.delegate_Lock gestureViewUnlockSuccess:self];
                    NSLog(@"成功");
                }
            ...
            { // 解锁失败
                ...
                if([self.delegate_Lock respondsToSelector:@selector(gestureViewUnlockFailed:)])
                {
                    [self.delegate_Lock gestureViewUnlockFailed:self];
                    NSLog(@"失败");
                }
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if(_blsetPassWord)
    {
        if (self.delegate_setLock && [self.delegate_setLock respondsToSelector:@selector(lockView:BeganTouch:)]) {
            [self.delegate_setLock lockView:self BeganTouch:touches];
        }
    }
